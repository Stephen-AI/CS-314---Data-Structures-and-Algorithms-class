...
// empty buffers into a music string
    ...
    {
        return solveb(e, v, 0, length(vars(e)));
    }
    public static Cons solveb(Cons e, String v, int count, int eqLen) {
      if(count >= 6*eqLen)
      return null;
      else if(stringp(lhs(e)))
      {
          if(lhs(e).equals(v))
          return e;
          else
          {
              Object trans = transformfp(algpats,e);             
              return solveb((Cons)trans, v, ++count, eqLen);
          }
       }
      else if(!consp(rhs(e)))
      {
            ...
            return solveb(list("=", rhs(e), lhs(e)), v, ++count, eqLen);            
      }
      else
      {
          Object trans = transformfp(algpats,e);         
    ...
    public static int conshash(Object e) {
        ...
        return (17 * conshash(first((Cons)e)) ^ (127 * conshash(rest((Cons)e))));
        else
        return e.hashCode();
    }
    ...
    public static int totaltime(Cons action) {
        ...
        switch((String)first(action))
        {
            ...
            case "kaboom":
            case "tambourine":
            return (int)(Integer)second(action);
            case "piano":
            return (int)(Integer)third(rest(action));
            case "seq":
            return totaltime((Cons)second(action)) + totaltime(cons("seq", rest(rest(action))));
            case "sync":
            return Math.max(totaltime((Cons)second(action)), totaltime(cons("sync",rest(rest(action)))));
            case "repeat":
            return (int)(Integer)second(action) * totaltime(cons("seq", rest(rest(action))));
            default:
            return 0;
        }
    }
    ...
    public static void execute(PriorityQueue<Event> pq, Cons act, int time) {
        ...
        else if(command.equals("cymbal"))
        {
         ...
         {
            int next = (int)((Integer)second(act) * random.nextDouble());
             if(next >= 2)
             {
             emit(11, time, 2);
             addevent(pq, list("tambourine", next), time + (Integer)second(act) - next);             
            }
            else
            return;
         }
         else if(command.equals("seq"))
         {
           ...
           if(did != null)
           {
           addevent(pq, (Cons)first(did), time);
           addevent(pq, cons("seq", rest(did)), time + totaltime((Cons)first(did)));
           return;
           }
           else
          // pq.poll();
         ...
         {
          execute(pq, list("seq", list("rest", (Integer)second(act)/4), list("boom", (Integer)second(act)/4), list("boom", (Integer)second(act)/2)), time);  
          return;
         }
         else if(command.equals("sync"))
         {
           Cons did = rest(act); 
           for(;did != null; did = rest(did))
           addevent(pq, (Cons)first(did), time);           
           return;
         }
         ...
         {
           Cons did = rest(act); 
           addevent(pq, (Cons)second(did), time);           
           if((int)first(did) > 1)
           addevent(pq, list("repeat", (Integer)first(did) - 1, (Cons)second(did)), time + totaltime((Cons)second(did)));
           return;
         }
        else if(command.equals("piano"))
        {
            Cons did = rest(act);
            emitp((int)(Integer)first(did), (String)second(did), time, (int)(Integer)third(did));
            return;
        }
    ...
    public static Cons round( Cons melody ) {
        Cons v1 = (Cons)copy_tree(melody);
        Cons v2 = (Cons)copy_tree(melody);
        melody = (Cons) subst(Integer.valueOf(0),"?i", melody);
         v1 = (Cons) subst(Integer.valueOf(1),"?i", v1);
        v2 = (Cons) subst(Integer.valueOf(2),"?i", v2);
        v1 = list("seq",list("rest", totaltime(melody)/4), v1);
        v2 = list("seq",list("rest", totaltime(melody)/2), v2);
        
        return list("sync", melody, v1, v2);
    }
    
    /*  
    public static Cons roundb(Cons melody, Cons v1, Cons v2, int i, int total)
    {
        if(rest(melody) == null)
        return melody;
        else if(totaltime((Cons)first(melody)) + i == total/4)
        return append(list((Cons)first(melody)), list("sync", "?j",roundb(rest(melody), null, v2,totaltime((Cons)first(melody)) + i, total)));
        else if(totaltime((Cons)first(melody)) + i == total/2)
        return append(list((Cons)first(melody)), list("sync", "?k",roundb(rest(melody), null, v2,totaltime((Cons)first(melody)) + i, total)));
        else
        return list((Cons)first(melody), roundb(rest(melody), v1, v2,totaltime((Cons)first(melody)) + i, total));     
...
*/
    ...
    public static void main( String[] args ) {
        ...
        Cons vset = null;
        while ( frm != null ) {
            System.out.println("   "  + ((Cons)first(frm)));
