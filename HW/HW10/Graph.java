...
public int astar( Vertex start, Vertex goal, Heuristic h ) 
 {
  ...
  PriorityQueue <Vertex>
      fringe = new PriorityQueue <Vertex> (20,
        new Comparator <Vertex> () {
          public int compare (Vertex i, Vertex j) {
              return ((i.cost + i.estimate) - (j.estimate + j.cost) ); }});
  fringe.add (start);
  while (! fringe.isEmpty ()) {     
    Vertex v = fringe.remove (); // lowest-cost
    ++count;
    ...
    if (!v.visited)
      {
        ...
        for (Edge e: v.edges)
          {
            int newcost = v.cost + e.cost;
            int newapprox = v.estimate + newcost;
            if (newcost < e.target.cost)
             {
               if (e.target.estimate < newapprox)
                 fringe.remove (e.target);
               e.target.cost = newcost;
               e.target.parent = v;
...
public static final Heuristic randombs = new Heuristic() {
   public Integer fn(Vertex from, Vertex to) {
          return (int)(rng.nextDouble() * from.distanceTo(to));
        } 
 };
...
public Cons pathto(String city) {
    Vertex dest = vertex(city);
    Cons path = Cons.list(city);
    while(dest.parent != null)
    {
        path = Cons.cons(dest.parent.contents, path);     
        dest = dest.parent;
    }
    return path;
 ...
 {
     //String format = "%-15s %-15s %-5s %-5s %n";
     String format = "%-20s %-20s %-20s %-20s %n";
     String format2 = "%-20s %-20s %-20s %-20s %n %n";
     HashMap<Vertex, Vertex> hm = new HashMap();
     hm.put(vertex("austin"), vertex("muleshoe"));
     hm.put(vertex("laredo"), vertex("haskell"));
     hm.put(vertex("dumas"), vertex("corsicana"));
     PrintWriter out = new PrintWriter("compare.txt");
     out.println(String.format(format, "JOURNEY","ALGORITHM","NODE COUNT","PATHLENGTH"));
     for(Vertex v: hm.keySet())
     {
     int nodes = dijkstra(v);
     out.println(String.format(format, v.contents + "-" + hm.get(v).contents, "Dijkstra", nodes, hm.get(v).cost()));
     prim(v);
     out.println(String.format(format, v.contents + "-" + hm.get(v).contents, "Prim", nodes, pathcost(hm.get(v))));
     out.println(String.format(format, v.contents + "-" + hm.get(v).contents, "A*-dist", astar(v, hm.get(v),dist), hm.get(v).cost()));
     out.println(String.format(format, v.contents + "-" + hm.get(v).contents, "A*-halfass", astar(v, hm.get(v),halfass), hm.get(v).cost()));
     out.println(String.format(format, v.contents + "-" + hm.get(v).contents, "A*-zip", astar(v, hm.get(v),zip), hm.get(v).cost()));
     out.println(String.format(format, v.contents + "-" + hm.get(v).contents, "A*-randombs", astar(v, hm.get(v),randombs), hm.get(v).cost()));
     out.println(String.format(format2, v.contents + "-" + hm.get(v).contents, "A*-randomlies", astar(v, hm.get(v),randomlies), hm.get(v).cost()));
