...
throws java.io.FileNotFoundException
 {
     ...
     // 1. line count
        ...
        final Mapper map1 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                mr.collect_map("line", list("1")); } };  // the Count!
    ...
    // ****** your code starts here ******
     ...
     // 2. letter count, case-insensitive
      String[] abc = {"a","b","c","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z"};
        final Mapper map2 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                for(String letter : abc)
                {
                 ...
                 if(letter.charAt(0) == Character.toLowerCase(value.charAt(i)))
                 mr.collect_map(letter , list("1")); 
                }
            } };
    
        final Reducer red2 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                //System.out.println("   red2 key = " + key + "  val = " + value);
                int sum = 0;
                for ( Cons lst = value; lst != null; lst = rest(lst) )
                    sum += Integer.decode((String) first((Cons)
                                                         first(lst)));
     ...
     // 3. word count, case-insensitive
        ...
        final Mapper map3 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                StringTokenizer tok = new StringTokenizer(value.trim(), " :,.;'-");
                while(tok.hasMoreTokens())
                mr.collect_map(tok.nextToken().toLowerCase(), list("1"));
                
            } };

        final Reducer red3 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                // System.out.println("   red3 key = " + key + "  val = " + value);
                int sum = 0;
                for ( Cons lst = value; lst != null; lst = rest(lst) )
                    sum += Integer.decode((String) first((Cons)
                                                         first(lst)));
        ...
        final Reducer red4 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
               //System.out.println("   red4 key = " + key + "  val = " + value);
                Cons pages = null;
                for ( Cons lst = value; lst != null; lst = rest(lst) )
                 pages = cons(first((Cons)first(lst)), pages);
                mr.collect_reduce(key, nreverse(pages));
            } };
        ...
        final Mapper map5 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                StringTokenizer tok = new StringTokenizer(value.trim(), " :,.;'-");
                while(tok.hasMoreTokens())
                mr.collect_map(tok.nextToken().toLowerCase(), list(key));
            } };

        final Reducer red5 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                // System.out.println("   red5 key = " + key + "  val = " + value);
                Cons pages = null;
                for ( Cons lst = value; lst != null; lst = rest(lst) )
                 pages = cons(first((Cons)first(lst)), pages);
                mr.collect_reduce(key, nreverse(pages));
     ...
     // 6. movie reviews
        ...
        final Mapper map6 = new Mapper() {
            public void map(String key, String value, MapReduce mr) {
                ...
                mr.collect_map(tok[0],list(tok[2]));
            } };
     // DecimalFormat formatter = new DecimalFormat("#0.00");
        final Reducer red6 = new Reducer() {
            public void reduce(String key, Cons value, MapReduce mr) {
                //System.out.println("   red6 key = " + key + "  val = " + value);
                Cons films = null;
                Double mean = 0.0;
                Integer ratings = length(value);
                for(Cons lst = value; lst != null; lst = rest(lst))
                {
                    mean = mean + (Integer.parseInt((String)first((Cons)first(lst)))/ (double)ratings);
                }
                 mr.collect_reduce(key, list(/*formatter.format(*/mean/*)*/, ratings));
            } };
